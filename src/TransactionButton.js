...
class TransactionButton extends React.Component
{
  ...
  render()
  {
    ...
    if (table === 2) //Other Tsumo
    {
      ...
      result = <button className="transaction" data-handsize={this.props.handsize} data-table={table} data-paymenteast={payment.east} data-paymentother={payment.other} data-honba={honba} onClick={e => this.props.onClick(e.target)}>{paymentWithHonba.other}<br />{paymentWithHonba.east}</button>;
    }
    else
    {
      if (this.props.limithand === "false")
      ...
      result = <button className="transaction" data-handsize={this.props.handsize} data-table={table} data-payment={payment} data-honba={honba} onClick={e => this.props.onClick(e.target)}>{paymentWithHonba}</button>;
    }
